...
function rehypeHydrateImages(options: HydrateImagesOptions) {
    return (tree: HastNode) => {
        ...
        for (const img of imgs) {
            const props = (img.properties ?? {}) as Record<string, unknown>;
...
const processor = (cdnBaseUrl: string) =>
    unified()
        .use(remarkParse)
        .use(remarkGfm)
        .use(remarkRehype)
        .use(rehypeHydrateImages, { cdnBaseUrl })
        .use(rehypeStringify);

/**
 * Convert a markdown body to HTML on the server with image blurhash
 * placeholders pre-baked in. Designed for TanStack loaders so the read-only
 * view can render straight to the DOM with no client-side parsing pass.
 */
export async function markdownToHtml(
    markdown: string,
    options: { cdnBaseUrl: string },
): Promise<string> {
    const file = await processor(options.cdnBaseUrl).process(markdown);
    return String(file);
}
