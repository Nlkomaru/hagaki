...
export async function processImage(file: File): Promise<ProcessedImage> {
    ...
    const resizedCanvas = drawToCanvas(bitmap, width, height);
    const imageData = canvasToImageData(resizedCanvas, width, height);
    ...
    if (avif.byteLength > MAX_AVIF_BYTES) {
...
async function makeBlurhash(source: ImageBitmap): Promise<string> {
    try {
        ...
        return encodeBlurhash(imageData.data, width, height, 4, 4);
    } catch (e) {
        throw new ImageProcessingError(
